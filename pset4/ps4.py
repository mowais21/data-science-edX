...
def evaluate_models_on_training(x, y, models):
    ...
    r2arr=[]
    for model in models:
        expectedarr.append(np.polyval(model,x))
    for expected in expectedarr:
        r2arr.append(r_squared(y, expected))
    best_ind = r2arr.index(max(r2arr))
    best_exp = expectedarr[best_ind]
    best_deg = len(models[best_ind]) - 1
    best_r2 = max(r2arr)
    pylab.figure()
    pylab.plot(x,y,'bo')
    pylab.plot(x,best_exp,'r')
#    pylab.xlabel('Year')
#    pylab.ylabel('Temperature')
    pylab.title('Degree of Regression Model: ' + str(best_deg) + '\n' + 'R-square value: ' + str(best_r2))
