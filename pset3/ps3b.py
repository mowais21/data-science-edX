...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            if i == 0:
                poplist.append([a.update()])
            else:
                poplist[j].append(a.update())
    for item in poplist:
        newpoplist.append(float(statistics.mean(item)))
    pylab.figure()
    pylab.title('Steps vs Population')
    pylab.xlabel('Population')
    pylab.ylabel('Steps')
    pylab.plot(newpoplist,x,label='Avg Population')
    pylab.legend()
    pylab.show()
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if self.isResistantTo(drug) == False:
                resistant = False
                break
        if (random.random() <= self.maxBirthProb * (1 - popDensity)) and resistant == True:
            for drug in rcopy:
                if rcopy[drug] == True:
                    if random.random() <= (1-self.getMutProb()):
                        ...
                        newr[drug] = False
                elif rcopy[drug] == False:
                    if random.random() <= (1-self.getMutProb()):
                        newr[drug] = False
                    else:
                        newr[drug] = True
            return ResistantVirus(self.getMaxBirthProb(),self.getClearProb(),newr,self.getMutProb())
        else:
            raise NoChildException
